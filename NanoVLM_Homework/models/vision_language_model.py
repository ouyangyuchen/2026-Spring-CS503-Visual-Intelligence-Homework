...
class VisionLanguageModel(nn.Module):
    ...
    def forward(self, input_ids, image, attention_mask=None, targets=None):
        ...
        if targets is not None:
            # Step 6: Project the embeddings to vocabulary distribution via decoder head
    ...
    def generate(self, input_ids, image, attention_mask=None, max_new_tokens=5):
        ...
        for i in range(max_new_tokens):
            ...
            if (next_token == self.cfg.lm_eos_token_id).all():  # (vii)
                break
    ...
    def generate_with_kv_cache(self, input_ids, image, attention_mask=None, max_new_tokens=5):
        ...
        for i in range(1, max_new_tokens):
            ...
            if (next_token == self.cfg.lm_eos_token_id).all():
                break
