...
class LanguageModelGroupedQueryAttention(nn.Module):
    ...
    def forward(self, x, cos, sin, attention_mask=None):
        ...
        if attention_mask is not None:
            ...
            attn_bias = attention_mask.unsqueeze(1).unsqueeze(2)
...
class LanguageModel(nn.Module):
    ...
    def forward_kv(self, x, past_key_values=None, attention_mask=None):
        if self.lm_use_tokens:
            x = self.token_embedding(x)

        B, T, _ = x.size()

        # Step 1: Fix position IDs for the decode phase
