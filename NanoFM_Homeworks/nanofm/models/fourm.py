...
class FourM(nn.Module):
    ...
    def forward_decoder(
        ...
        x = self.decoder(x, context=context, sa_mask=dec_pad_sa_mask, xa_mask=dec_pad_xa_mask)
    ...
    def generate_one_modality_roar(
        ...
        random_order_positions = torch.randperm(n_tokens_target, device=device)
        ...
        dec_input_positions_list = [chunk.unsqueeze(0) for chunk in torch.split(random_order_positions, schedule)]
        ...
        for step, k in enumerate(schedule):
            ...
            predicted_logits = self.forward_model(
                ...
                dec_input_positions=dec_input_positions,
            )[0]
