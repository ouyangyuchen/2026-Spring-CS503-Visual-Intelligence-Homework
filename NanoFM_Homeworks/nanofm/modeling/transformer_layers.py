...
class Attention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        q, k, v = rearrange(self.qkv(x), "b l (qkv h d) -> qkv b h l d", qkv=3, h=self.num_heads)
        ...
        if mask is not None:
            attn_mask = rearrange(mask, "b n m -> b 1 n m").to(torch.bool) # Unsqueeze for multi-head attention
...
class CrossAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, context: torch.Tensor, mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        k, v = rearrange(self.kv(context), "b m (kv h d) -> kv b h m d", kv=2, h=self.num_heads)
...
class DecoderBlock(nn.Module):
    ...
    def __init__(self, dim: int, head_dim: int = 64, mlp_ratio: float = 4., use_bias: bool = False):
        ...
        mlp_hidden_dim = int(dim * mlp_ratio)
        self.mlp = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, bias=use_bias) # TODO MLP layer
    ...
    def forward(self, 
        ...
        x = x + self.cross_attn(self.query_norm(x), context=self.context_norm(context), mask=xa_mask)
        x = x + self.mlp(self.norm2(x))
        return x
...
class TransformerDecoderTrunk(nn.Module):
    ...
    def forward(
        ...
        for block in self.blocks:
            x = block(x, context=context, sa_mask=sa_mask, xa_mask=xa_mask)
        return x
